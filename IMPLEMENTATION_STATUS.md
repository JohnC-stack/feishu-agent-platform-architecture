...
| 任务                     | 状态   | 当前证据                                                                                              |
| ------------------------ | ------ | ----------------------------------------------------------------------------------------------------- |
| `P6-01` 核心运行页面     | 已完成 | 总览、任务与会话、队列与 Worker、执行器与沙箱均使用真实聚合数据                                       |
| `P6-02` 治理与配置页面   | 已完成 | 集成、审批、用户与角色、Token/成本、配置页面已接入 RBAC；角色绑定管理、服务端过滤和页面验收均已通过   |
| `P6-03` 可观测性与告警   | 已完成 | 日志与 Trace、健康、失败任务、模型失败、队列积压、服务离线和预算告警已实现                            |
| `P6-04` 发布、备份与恢复 | 已完成 | 发布、备份、配置版本和恢复记录具备数据表、聚合 API 与可视化页面                                       |
| `P6-05` 受治理运维操作   | 已完成 | 取消、重试、清理、重启和回滚要求精确中文确认；高风险操作审批前保持 `pending_approval`，所有写入均审计 |
