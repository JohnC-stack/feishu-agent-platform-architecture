...
本仓库记录一套面向企业内网的飞书 Agent 平台设计。用户在飞书群聊或私聊中与机器人交互，平台按任务特征选择确定性工具、API/ReAct 或本地 Agent CLI，并统一治理身份、权限、审批、Token 成本和执行审计。

## 当前状态

项目已经进入实施阶段。P0 工程基线和 P1 飞书接入 PoC 已完成本地、真实飞书和远端全量验收并关闭，当前进入 P2 调度与数据阶段。已完成内容包括：
...
10. PostgreSQL、配置和审计数据进行加密备份，并定期验证恢复。
...
P0、P1 已关闭。当前开始 P2：建设任务状态机、BullMQ 队列、版本化规则路由、会话上下文和 PostgreSQL 持久化，并以并发、幂等、重试和异常恢复测试作为阶段出口。
